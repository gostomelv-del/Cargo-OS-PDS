...
func TestVerifierRejectsTamperingUnknownAndRevokedKeys(t *testing.T) {
	...
	}, signedAt); !errors.Is(err, ErrVerificationKeyAbsent) {
		t.Fatalf("expected unknown key rejection, got %v", err)
	}
	if _, err := verifier.Verify(context.Background(), version, Signature{
		SignerID: "policy-authority", KeyID: "revoked", Algorithm: AlgorithmEd25519,
		SignedAt: signedAt, Value: value,
	}, signedAt); !errors.Is(err, ErrKeyRevoked) {
	...
	}, signedAt); !errors.Is(err, ErrInvalidSignature) {
...
func TestVerifierRejectsNewAdmissionAfterRevocation(t *testing.T) {
	signedAt := time.Date(2026, 7, 20, 12, 0, 0, 0, time.UTC)
	version, _ := NewVersion(policyInput("1.0.0", signedAt, nil))
	privateKey := ed25519.NewKeyFromSeed(make([]byte, ed25519.SeedSize))
	store, _ := NewMemoryTrustStore(VerificationKey{
		SignerID: "policy-authority", KeyID: "rotated", Algorithm: AlgorithmEd25519,
		PublicKey: privateKey.Public().(ed25519.PublicKey), ValidFrom: signedAt.Add(-time.Hour),
	})
	signature := Signature{
		SignerID: "policy-authority", KeyID: "rotated", Algorithm: AlgorithmEd25519, SignedAt: signedAt,
	}
	payload, _ := SigningPayload(version, signature)
	signature.Value = base64.StdEncoding.EncodeToString(ed25519.Sign(privateKey, payload))
	verifier, _ := NewVerifier(store)
	if _, err := verifier.Verify(context.Background(), version, signature, signedAt); err != nil {
		t.Fatal(err)
	}
	revokedAt := signedAt.Add(time.Minute)
	if err := store.Revoke("policy-authority", "rotated", revokedAt); err != nil {
		t.Fatal(err)
	}
	if _, err := verifier.Verify(context.Background(), version, signature, revokedAt); !errors.Is(err, ErrKeyRevoked) {
		t.Fatalf("expected post-revocation admission rejection, got %v", err)
	}
}
