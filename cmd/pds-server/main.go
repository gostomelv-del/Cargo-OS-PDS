...
func run() error {
	...
	server := &http.Server{
		Addr:              address,
		Handler:           httpapi.NewHandlerWithEvidence(service, evidenceService, readiness),
...
func newService(
...
) (*pds.Service, *evidence.Service, httpapi.ReadinessChecker, func(), error) {
	if databaseURL == "" {
		...
		return pds.NewService(nil), evidenceService,
	...
	return pds.NewServiceWithStore(store, nil), evidenceService, postgresReadiness(db), closeDatabase, nil
}
...
func postgresReadiness(db *sql.DB) httpapi.ReadinessChecker {
	return httpapi.ReadinessFunc(func(ctx context.Context) error {
		...
		defer cancel()
		var evaluations, history, outbox, evidenceObjects, policyVersions bool
		...
		`).Scan(&evaluations, &history, &outbox, &evidenceObjects, &policyVersions)
		if err != nil {
			return fmt.Errorf("readiness query: %w", err)
		}
		if !evaluations || !history || !outbox || !evidenceObjects || !policyVersions {
